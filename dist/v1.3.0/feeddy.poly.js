var feeddy = (function (exports) {
  ...
  function fetch$1(input, init) {
    if (typeof XMLHttpRequest === 'undefined')
      throw new ReferenceError(
        'This fetch polyfills requires XMLHttpRequest. You must call this on the browser.',
      );
  ...
  function requiredWithType(value, type, name) {
    ...
    if (isArray(type)) {
      if (
        type.every(function (it) {
          return !typeIs(value, it);
        })
      )
        throw new IllegalArgumentError(
          concat(
            'The ',
            name,
            ' argument must be be one of these types: [',
            type,
            '[]',
          ),
        );
      ...
      throw new IllegalArgumentError(
        concat('The ', name, ' argument must be a ', type),
      );
    return value;
  }
  ...
  var Promise$1 = funclass2({
    construct: function (executor) {
      ...
      try {
        executor(bind(resolve$1, $this), rej);
      } catch (e) {
        rej(e);
      }
    },
    ...
    prototype: {
        ...
        return new Promise$1(function (resolve, reject) {
          switch (get$1($this, promiseState)) {
            case 'pending':
              set($this, promiseCalls, [
                finallyPromise($this, resolve, reject, onFinally),
              ]);
              break;
            case 'fulfilled':
            case 'rejected':
      ...
      then: function (onfulfilled, onrejected) {
        ...
        return new Promise$1(function (resolve, reject) {
          var res = resolverPromise(resolve, reject, onfulfilled),
            ...
            result = get$1($this, promiseResult);
          switch (get$1($this, promiseState)) {
            case 'pending':
