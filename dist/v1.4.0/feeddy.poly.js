...
var feeddy = function(exports) {
    ...
    var nullable = null, indefinite = undefined;
    function typeIs(type, value) {
    ...
    function bind(fn, thisArg) {
        return apply(fn.bind, fn, concat([ thisArg ], slice(arguments, 2)));
    }
    function binds(fn) {
        return function(thisArg) {
            return apply(fn, thisArg, slice(arguments, 1));
        };
    }
    var isFunction = bind(typeIs, indefinite, "function"), isNumber = bind(typeIs, indefinite, "number"), isString = bind(typeIs, indefinite, "string");
    function caller(key) {
    ...
    var O = Object, keys = O.keys, propertyNames = O.getOwnPropertyNames, defineProperty = O.defineProperty, freeze = O.freeze, valueOf = caller("valueOf"), prototype$4 = property("prototype");
    var forEach = binds(prototype$4(Array).forEach);
    function keach(source, each, thisArg) {
        each = bind(each, thisArg);
        var index = 0;
        for (var key in source) {
            each(source[key], key, index);
            index++;
        }
    }
    function multiple(definer, target, descriptors) {
        keach(descriptors, (function(descriptor, key) {
            return definer(target, key, descriptor);
        }));
    }
    ...
    var props = bind(multiple, indefinite, prop);
    function _value(writable, numerable, configurable, target, key, value) {
        prop(target, key, descriptor(value, writable, configurable, numerable));
    }
    var readonly = bind(_value, indefinite, indefinite, indefinite, indefinite), readonly2 = bind(_value, indefinite, indefinite, true, indefinite), writeable = bind(_value, indefinite, true, indefinite, indefinite), readonlys2 = bind(multiple, indefinite, readonly);
    var A = prototype$4(Array), reduce = binds(A.reduce);
    ...
    function reduceText(text, length, separator, joiner) {
        ...
        for (var i = 0; i < len(source); i++) {
            ...
            if (length && len(value) > length + 1) break;
            result = value;
        }
    ...
    function get$1(object, key) {
        ...
        for (;i < len(args); i++) {
            key = args[i];
            if (object) object = object[key]; else return object;
        }
        return object;
    }
    ...
    function setTo(object, key, target) {
        ...
        } else if (!isObject(key)) return target; else props = key;
    ...
    function statics(target, base) {
        (get2(Object, "setPrototypeOf") || {
            __proto__: []
        } instanceof Array && function(target, base) {
    ...
    function prototype$3(target, base) {
        if (!isFunction(base) && base !== nullable) throw new TypeError(concat("Class extends value ", base, " is not a constructor or null"));
        statics(target, base);
        function __() {
            this.constructor = base;
        }
        try {
            target.prototype = base === nullable ? Object.create(base) : (__.prototype = base.prototype, 
            new __);
        } catch (e) {
    ...
    function parentFirst(constructor, parent) {
        return parent ? function Instanceable() {
    ...
    function funclass2(options, parent) {
        ...
        if (!isFunction(clsBuilder)) throw Exception("The modified constructor builder must be a function.");
        var cls = clsBuilder(constructor, parent);
        if (isDefined(init) && !isFunction(init)) throw Exception("The constructor must be a function.");
        if (isDefined(cls) && !isFunction(cls)) throw Exception("The modified constructor must be a function.");
        init = cls || init || function() {
            return this;
        };
        ...
        if (!call(includes, acceptedTypes, typeof funPrototype)) throw Exception("The function prototype must be an object.");
    ...
    function withType(type, value, name) {
        name = name || "value";
        var validator = bind(typeIs, indefinite, type);
        if (isArray(type)) {
            if (type.every(validator)) throw new IllegalArgumentError(concat("The ", name, " argument must be be one of these types: [", type, "[]"));
        } else if (!validator(value)) throw new IllegalArgumentError(concat("The ", name, " argument must be a ", type));
        return value;
    }
    var requireFunction = bind(withType, indefinite, "function");
    ...
    function fetch$1(input, init) {
        if (typeof XMLHttpRequest === "undefined") throw new ReferenceError("This fetch polyfills requires XMLHttpRequest. You must call this on the browser.");
    ...
    var M = Math, max$1 = M.max, min = M.min, round = M.round, random = M.random;
    var id = 0;
    var postfix = random();
    function uid(key) {
        return concat("Symbol('", string(key), "')_", apply(1..toString, ++id + postfix, [ 36 ]));
    }
    function partial(fn) {
        var source = slice(arguments, 1);
        return function() {
            return apply(fn, this, concat(source, slice(arguments)));
        };
    }
    ...
    function resolveOrReject(state, index, value) {
        var $this = this;
        if (get2($this, promiseState) !== "pending") return;
        if (value && isFunction(value.then)) {
            var rej = bind(rejectHelper, $this), res = bind(resolveHelper, $this);
            if (value === $this) rej(new TypeError("Chaining cycle detected for promise.")); else value.then(res, rej);
            return;
        }
        set($this, promiseState, state);
        set($this, promiseResult, value);
        if (!hasOwn($this, promiseCalls)) return;
        var calls = get2($this, promiseCalls);
        if (len(calls) === 1) calls[0](value); else calls[index](value);
    }
    var resolveHelper = partial(resolveOrReject, "fulfilled", 0), rejectHelper = partial(resolveOrReject, "rejected", 1);
    ...
    var Promise$1 = funclass2({
        construct: function(executor) {
            ...
            var $this = this, rej = bind(rejectHelper, $this);
            ...
            all: function(values) {
                return new Promise$1((function(resolve, reject) {
                    ...
                    forEach(values, (function(promise, index) {
                        Promise$1.resolve(promise).then((function(result) {
        ...
        prototype: {
            ...
            then: function(onfulfilled, onrejected) {
                var $this = this;
                return new Promise$1((function(resolve, reject) {
                    var res = resolverPromise(resolve, reject, onfulfilled), rej = resolverPromise(reject, reject, onrejected), result = get2($this, promiseResult);
                    switch (get2($this, promiseState)) {
                      case "pending":
            ...
            toString: function() {
                return concat("Promise { <", get2(this, promiseState), "> }");
            }
        }
    });
    ...
    var dateTypes = [ "published", "updated" ], parametersMap = {
        "max-results": "max",
        "start-index": "start",
    ...
    function updateProperty(args, source, key, builder, allowUndefined) {
        ...
        if (!allowUndefined && !isDefined(value)) deletes(source, key); else source[key] = builder ? builder(value) : value;
        return source[key];
    }
    ...
    var prototype$2 = {
        ...
        query: propertyFn("q"),
        alt: propertyFn("alt", (function(it) {
            return call(includes, [ "json", "rss", "atom" ], it) ? it : "json";
        }), true),
        orderby: propertyFn("orderby", (function(order) {
            return call(includes, dateTypes, order) ? order : "updated";
        }), true),
        toDefined: partial(simple, "source")
    };
    ...
    var SearchParams = funclass2({
        construct: function(source) {
            source = source || {};
            for (var key in source) {
                (!parametersMap[key] || !isDefined(source[key])) && deletes2(source, [ key ]);
            }
    ...
    function paramsFrom(source, copy) {
        if (isinstance(source, SearchParams)) return copy ? paramsFrom(source.source, copy) : source;
        return new SearchParams(copy ? assign2({}, source) : source);
    }
    function paramIndex(action, index) {
    ...
    function paramDate($this, min, max, type, keepAtLeast, keepAtMost) {
        ...
        (isDefined(min) || !keepAtLeast) && apply(get$1(params, type + "AtLeast"), params, [ min ]);
        (isDefined(max) || !keepAtMost) && apply(get$1(params, type + "AtMost"), params, [ max ]);
        return $this;
    }
    ...
    var searchParamsSymbol = uid("p"), realMax = uid("m"), maxResults = 500, source = [ "place", "plus", "minus" ].map((function(mode) {
        return partial(paramIndex, mode);
    })), max = function(value) {
        var $this = this;
        value = get$1($this, searchParamsSymbol).max(value);
        ...
        paginated: page,
        page: page,
        repage: function(total, page) {
            var $this = this;
            if (isDefined(total) && isDefined(page)) {
                total = toInt(nullable, total);
                page = toInt(nullable, page);
                var mx = get$1($this, realMax), index = coerceAtLeast(1, total - page * mx + 1), params = get$1($this, searchParamsSymbol), prevIndex = mx;
                if (page > 1) prevIndex = coerceAtLeast(1, total - (page - 1) * mx + 1 - index);
    ...
    function createRoute(type, route, id) {
        var suffix = route === "full" ? "full" : "summary", mid = "posts", prefix = "/";
        if (type === "pages") mid = "pages"; else if (type === "comments") {
            mid = "comments";
    ...
    function buildUrl(options, id) {
        ...
        var href;
        if (isDefined(options.blogUrl)) href = options.blogUrl; else if (typeof location !== "undefined") href = location.origin; else throw new IllegalArgumentError("You must pass the blog url or call this on the browser.");
    ...
    function getId(source, type) {
        ...
        if (!apply(includes, [ "blog", "post", "page" ], [ type ])) throw new IllegalArgumentError(concat("'", type, "' is an unknown type id."));
        var expr = new RegExp(concat(type, "-([0-9]+)"), "g"), res = expr.exec(source);
        return res ? res[1] : "";
    }
    ...
    function _rawGet(options, all, id) {
        ...
        var entries = [], url = buildUrl(options, id), startIndex = params.start() || 1;
        function request(url, max) {
            return fetch(string(url)).then((function(res) {
                if (res.status !== 200) throw new IllegalAccessError("Request failed. Status: " + res.status);
                return res.text();
            })).then((function(body) {
                var blog;
                try {
                ...
                feed.entry = entries;
                if (params.max() !== len(entries)) feed.openSearch$itemsPerPage.$t = feed.openSearch$totalResults.$t = string(len(entries));
    ...
    function rawCategoryToCategory(text) {
        return string(get$1(text, "term"));
    }
    function rawCategoriesToCategories(category) {
    ...
    function rawAuthorsToAuthors(author) {
        return isArray(author) ? author.map(rawAuthorToAuthor) : [];
    }
    ...
    function byId(options) {
        var mapper = isComments(get2(options, "feed")) ? rawBlogToBlog : rawBlogEntryToBlogEntry;
    ...
    function entriesBase(options, builder, id) {
        ...
        var params = paramsFrom(feed.params), paramsBuilder = builderFrom(params);
        var request = isComments(feed) && id ? function(feed) {
            return _rawGet(feed, params.query(), id).then(rawBlogToBlog);
        } : get, createHandler = builder(feed, params, paramsBuilder, request);
        params.query() || params.max(1);
        return (id && isComments(feed) ? request : params.query() ? all : get)(feed).then(createHandler);
    }
    ...
    function basicHandlerPage(feed, _, builder, request) {
        return function(page, reverse) {
            reverse ? builder.repage(this.total, page) : builder.page(page);
            feed.params = builder.build();
            return request(feed).then((function(blog) {
                var entries = get$1(blog, "feed", "entry") || [];
    ...
    function posts(options) {
        ...
        return entriesBase(options, basicHandler(basicHandlerPage, (function(blog) {
            return {
    ...
    function appendQuery(name) {
        var $this = this, qt = quote(get$1($this, exactSymbol)), op = operator(get$1($this, operatorSymbol)), xc = get$1($this, excludeSymbol) ? exclude() : "", args = slice(arguments, 1);
        if (isEmpty(args)) return $this;
        name = string(name);
        name = isNotEmpty(name) ? concat(name, ":") : "";
        var current = get$1($this, querySymbol), query = buildQuery(args, concat(qt, op, xc, name, qt), concat(xc, name, qt), qt);
    ...
    var named = [ "label", "title", "author", "link" ].map((function(key) {
        var handler = partial(appendQuery, key);
    ...
    function splitQuery(query) {
        ...
        while ((match = tokenRegex.exec(query)) !== null) {
            if (match[1] && match[2]) {
                ...
                named || (named = result.named[key] = {
                    exact: [],
                    terms: []
                });
                named.exact.push(value);
            } else if (match[3] && match[4]) {
                var key = match[3], value = match[4];
                var named = result.named[key];
                named || (named = result.named[key] = {
                    exact: [],
                    terms: []
    ...
    function parseSize(constructor, isSize, format, ratio) {
        var aspectRatio = 1;
        if (!isDefined(ratio)) aspectRatio = parseSize(constructor, isSize, format, 0).ratio(); else if (isNumber(ratio)) aspectRatio = ratio; else if (!isSize(ratio)) aspectRatio = parseSize(constructor, isSize, ratio, 0).ratio(); else if (isFunction(get2(ratio, "ratio"))) aspectRatio = ratio.ratio();
        ...
        } else if (isDefined(width)) height = width / (aspectRatio || 1); else if (isDefined(height)) width = height * (aspectRatio || 1); else throw new IllegalArgumentError(concat("The format ", format, " is not valid."));
    ...
    function postThumbnail(source, size, ratio) {
        ...
        return source.replace(sizeMatch, concat("/", expression, "/")).replace(legacyMatch, concat("=", expression));
    }
    function descsort(key) {
        return isDefined(key) ? function(a, b) {
            return b[key] - a[key];
        } : function(a, b) {
            return b - a;
        };
    ...
    function withCategories(options) {
        ...
        if (isEmpty(categories)) return reject("The categories are empty.");
        var feed = feedOptions(options.feed), params = paramsFrom(feed.params), builder = queryBuilder().exact();
        set(feed, "type", "posts");
        apply(options.every ? builder.and : builder.or, builder);
        builderFrom(params).query(apply(builder.categories, builder, categories).build());
        return all(feed).then((function(blog) {
            return freeze({
                posts: blog.feed.entry.map((function(post) {
                    return {
                        count: len(post.category.filter((function(it) {
                            return categories.indexOf(it) >= 0;
                        }))),
                        post: post
                    };
                })).sort(descsort("count")).slice(0, params.max()),
                blog: blog
            });
        }));
    }
    ...
    function commentsById(options) {
        ...
        return entriesBase(options, basicHandler(basicHandlerPage), get$1(options, "id"));
    }
    ...
    function entryPathname(source, length) {
        ...
        if (links && isNotEmpty(links)) {
            link = apply(findLast, links, [ function(value) {
    ...
    function createResult(base, parameters) {
        ...
        base = isEmpty(values) ? base : concat(base, "?", values.sort((function(a, b) {
    ...
    function ssrPosts(options) {
        set(options, "feed", "type", "posts");
        var ssr = get$1(options, "ssr"), label = get$1(options, "category"), sourceParams = get$1(options, "feed", "params");
        var query = get$1(sourceParams, "q"), url = "/search";
        ...
        return entriesBase(options, basicHandler((function(feed, params, builder, request) {
            var max = params.max(), query = params.query();
            return function(page) {
                var baseUrl = url, parameters = {
                    "max-results": max
                }, early = indefinite;
                builder.max(max).page(page).minusIndex(1);
                if (ssr === "query" && query) {
                    parameters["q"] = query;
